...
class FighterRetriever:
    def __init__(self):
        ...
        if len(parts) >= 2:
            # Only keep the full first+last name.
            # Never index just the first name.
            self.name_variants[f"{parts[0]} {parts[-1]}"] = idx
    ...
    def get_fighter(self, fighter_name: str):

        idx = self.name_to_index.get(fighter_name.lower())

        if idx is None:
            return None

        return {
            "name": self.fighter_names[idx],
            "stats": self.metadata[idx].copy(),
            "document": self.documents[idx],
        }
